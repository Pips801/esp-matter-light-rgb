...
extern "C" void app_main()
{
    ...
    app_driver_handle_t temp_handle = app_driver_temp_init();
    ...
    /* Create Extended color endpoint */
    ...
    ESP_LOGI(TAG, "light_config added to endpoint %d" , endpoint::get_id(endpoint));


    /* create Temperature sensor endpoint*/
    temperature_sensor::config_t temp_config;
    temp_config.temperature_measurement.measured_value = 420;
    temp_config.temperature_measurement.max_measured_value = 6969;
    temp_config.temperature_measurement.min_measured_value = 69;
    endpoint_t *endpoint1 = temperature_sensor::create(node, &temp_config, ENDPOINT_FLAG_NONE, NULL);
    ESP_LOGI(TAG, "temp_config added to endpoint %d" , endpoint::get_id(endpoint1));

    /* These node and endpoint handles can be used to create/add other endpoints and clusters. */
    if (!node || !endpoint) {
        ESP_LOGE(TAG, "Matter node creation failed");
    } else {
        ESP_LOGI(TAG, "Matter node creation succeeded.");
    }

    light_endpoint_id = endpoint::get_id(endpoint);
    ESP_LOGI(TAG, "Light created with endpoint_id %d", light_endpoint_id);

    /* Add color control feature to lighting cluster */
    ...
    if (err != ESP_OK) {
        ...
        ESP_LOGI(TAG, "Matter started without error.");
    }

    /* Starting driver with default values */
    ESP_LOGI(TAG, "Starting driver with default values at endpoint ID: %d", light_endpoint_id);
